...
namespace AkkaCircuitBreaker_InCS_Tests
{
    ...
    /// </summary>
    [TestClass]
    public class Given_A_Synchronous_Circuit_Breaker_That_Is_HalfOpen : CircuitBreakerTestKit
    {
        [TestMethod]
        public void When_Next_Called_Then_Pass_Call_And_Transition_To_Close_On_Success( )
        {
            var breaker = ShortResetTimeoutCb( );
            Intercept<TestException>( ( ) => breaker.Instance.WithSyncCircuitBreaker( ThrowException ) );
