...
namespace AkkaCircuitBreaker_InCS_Tests
{
    ...
    /// </summary>
    [TestClass]
    public class Given_An_Asynchronous_Circuit_Breaker_That_Is_Open : CircuitBreakerTestKit
    {
        [TestMethod]
        public void When_Called_Before_Reset_Timeout_Then_Throw_Exceptions( )
        {
            ...
            Assert.IsTrue( Intercept<OpenCircuitException>( ( ) => breaker.Instance.WithCircuitBreaker( Task.Factory.StartNew( ThrowException ) ).Wait( ) ) );
        }
